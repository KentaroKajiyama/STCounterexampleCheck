...
$LeanExecPath = "C:\Users\jesus\OneDrive\Documents\VScode\Repositories\research\SymmetricTensorProof\.lake\build\bin\verify_proof.exe" # Leanの実行ファイル名
$T_VAL = 6
$P_VAL = 2147483647 # 2^31 - 1
...
foreach ($File in $BinFiles) {
    ...
    if (Test-Path -Path $TempOutputFile) {
        ...
        try {
            ...
            # 例: & $LeanExecPath "LeanMainFile.lean" "--input=$TempOutputFile"
            & $LeanExecPath $TempOutputFile $T_VAL $P_VAL | Out-Host 
