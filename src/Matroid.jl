...
"""
    find_circuit(M_all::STM, indices_g::Vector{Int})
...
"""
function find_circuit(M_all::STM, indices_g::Vector{Int})
    ...
    for k in indices_g
        ...
        subM_all = M_all[:, B_indices]
        r = compute_rank(subM_all)
...
"""
function find_closure(g::AbstractGraph, M::STM, C_indices::Vector{Int})
    # F (閉包) の初期値は C
    ...
    # K_n の全エッジに対して判定 (定義上、K_n \ C だけでなく C 内もチェックしてよいが、
    # 効率のため C に含まれないものだけをチェックする)

    all_indices = 1:size(M, 2)
